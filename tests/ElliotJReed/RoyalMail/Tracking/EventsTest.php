...
final class EventsTest extends TestCase
{
    ...
    public function testItThrowsResponseErrorWhenInvalidJsonReturnedFromApiWhenHttpErrorsSetToFalse(): void
    {
        $mock = new MockHandler([new Response(200, [], 'NOT JSON')]);
        $client = new Client(['handler' => HandlerStack::create($mock), 'http_errors' => true]);
    ...
    public function testItThrowsRoyalMailServerErrorWhenErrorIsUnexpectedJson(): void
    {
        $mock = new MockHandler([
            new Response(500, [], '{"something":"unexpected"}')
        ]);
        $client = new Client(['handler' => HandlerStack::create($mock), 'http_errors' => true]);

        $this->expectException(RoyalMailServerError::class);
        $this->expectExceptionMessage('(500) {"something":"unexpected"}');

        (new Events($client, 'client-id', 'client-secret'))
            ->setTrackingNumber('123456789GB');
    }

    public function testItThrowsRoyalMailServerErrorWhenErrorIsUnexpectedJsonAndHttpErrorsSetToFalse(): void
    {
        $mock = new MockHandler([
            new Response(500, [], '{"something":"unexpected"}')
        ]);
    ...
    public function testItThrowsInvalidCredentialsExceptionWhenGuzzleExceptionsOnAndBadRequestHttpStatus(): void
    {
        ...
        try {
            ...
            $this->assertSame('(Error code) Error description', $exception->getMessage());
